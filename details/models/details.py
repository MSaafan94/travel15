...
class SaleOrderr(models.Model):
    ...
    def get_sale_order_details(rec):
        sales_details = [(5, 0, 0,)]
        for self in rec:
            if self.partner_id:
...
class SaleOrderAccommodation(models.Model):
    ...
    no_of_nights = fields.Char(string='No. Of Nights', compute='_no_of_nights', store=True)
    ...
    def _no_of_nights(self):
        ...
        for rec in self:
            if rec.check_in_date and rec.check_out_date:
                d1 = rec.check_in_date
                d2 = rec.check_out_date
                g = str(d2 - d1).split()[0]
                if g == '0:00:00':
                    rec.no_of_nights = '0 night'
                else:
                    rec.no_of_nights = g + 'night'

    # @api.one
    def _age_on_travel_date(self):
        # if self.partner_id & self.partner_id.birthday:
            for rec in self:
                total_days = rec.sale_id.endtime.date() - rec.partner_id.birthday
                years = int(abs(total_days.days / 365))
                remaining_days = total_days.days % 365
                if remaining_days >= 30:
                    months = int(abs(remaining_days / 30))
